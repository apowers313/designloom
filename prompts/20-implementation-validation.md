...
1. **Select Workflow to Validate**:
   ...
   **Step 3: If no workflows need validation**
   ...
   - Running prompt 19 to implement the next workflow
   - Running prompt 21 to check overall progress
...
7. **Check Prior Test Results**:
   ```
   design_list_test_results --workflow_id [SELECTED_WORKFLOW_ID]
   ```
...
8. **Record Validation as TestResult**:
   ...
   ```yaml
   design_create_test_result --id TR-[WORKFLOW_ID]-[persona-id]-[sequence] --workflow_id [WORKFLOW_ID] --persona_id [relevant-persona] --test_type real --date [TODAY] --status [passed|failed|partial] --confidence high --success_criteria_results '[
...
9. **Update Workflow Status**:
   ...
   **If PASSED**:
   ...
   design_update_workflow --id [SELECTED_WORKFLOW_ID] --data '{
     "validated": true,
     "notes": "IMPLEMENTED - VALIDATED [DATE]. All success criteria met. TestResult: TR-[ID]"
   }'
   ```
   ...
   **If FAILED or PARTIAL**:
   ```yaml
   design_update_workflow --id [SELECTED_WORKFLOW_ID] --data '{
     "notes": "IMPLEMENTED - VALIDATION FAILED [DATE]. Issues: [summary]. TestResult: TR-[ID]"
   }'
   ```
...
10. **Update Designloom with Learnings**:
    ...
    design_create_source --data '{
      ...
      "summary": "Changed X because Y. Approved by [who]."
    }'
    ```
...
- TestResult created with `design_create_test_result`
